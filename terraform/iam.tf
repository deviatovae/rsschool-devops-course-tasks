resource "aws_iam_policy" "terraform_backend" {
  ...
  description = "Access to S3 bucket for Terraform backend"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Effect = "Allow",
      ...
      Resource = [
        "arn:aws:s3:::aldev-terraform-bucket",
        "arn:aws:s3:::aldev-terraform-bucket/*"
      ]
    }]
  })
...
resource "aws_iam_role" "github_actions" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringLike = {
          "token.actions.githubusercontent.com:sub" = "repo:deviatovae/rsschool-devops-course-tasks:*"
        }
      }
    }]
  })
